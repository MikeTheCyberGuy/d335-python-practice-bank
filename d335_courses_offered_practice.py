# File: d335_courses_offered_practice.py

"""
Written by Michael Briley.
D335 Practice Question – Courses Offered Summary
...
1. Works with a text file named: courses.txt

   The file contains one course name per line.
   Example contents of courses.txt:
       Math 101
       History 201
       Cybersecurity 335
...
2. Your program must:
   ...
   - Store the cleaned course names in a list.
   - Build a single summary line in this exact format:

       Courses offered: Math 101, History 201, Cybersecurity 335
...
# Hints:
#   - Use a variable for the file name, e.g. filename = "courses.txt"
#   - Use open(..., "r") and .readlines() to read all the lines.
#   - Strip each line (e.g. with .strip()) to remove "\n" and spaces.
#   - Store the cleaned lines in a list called courses (or similar).
#   - Use ', '.join(courses) to build a single string of all course names.
...
def reference_solution():
    """Reference implementation for the Courses Offered Summary practice question."""
    ...
    # Open the file in read mode ("r") to get the existing course names.
    ...
    # After this, we might get: ["Math 101", "History 201", "Cybersecurity 335"]
    courses = [content.strip() for content in contents]
    ...
    with open(filename, "a") as f:
        # ', '.join(courses) creates a single string with all course names
        # separated by a comma and a space.
        #
        # Example:
        #   courses = ["Math 101", "History 201", "Cybersecurity 335"]
        #   ', '.join(courses) -> "Math 101, History 201, Cybersecurity 335"
        #
        # We add a leading "\n" so the summary starts on a new line,
    ...
    # so the user can see both the original course list and the new summary line.
    with open(filename, "r") as f:
        print(f.read())
